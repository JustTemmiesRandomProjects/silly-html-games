...
export function drawBackgroundImage(context, image) {
    ...
    const scale = Math.max(context.canvas.width / image.width, context.canvas.height / image.height);
...
export function sound(src, key) {
    ...
    if (!key.includes("_")) {
        console.log(
            `the ID ${key} isn't valid, it needs to have a category, like "sound_" or "music_" before the actual name`
        );
        alert(
            `the ID ${key} isn't valid, it needs to have a category, like "sound_" or "music_" before the actual name`
        );
    ...
    if (settings.volume_mixer[this.category] == undefined) {
        console.log(
            `the category "${this.category}" was not found in settings.volume_mixer, this is not a valid category for ${key}`
        );
        alert(
            `the category "${this.category}" was not found in settings.volume_mixer, this is not a valid category for ${key}`
        );
...
export class GIF {
    ...
    draw() {
        ...
        if (this.scale != 1) {
            ...
            this.ctx.drawImage(
                this_frame,
                this.position["x"] - this_frame.width / 2,
                this.position["y"] - this_frame.height / 2
            );
        }
    }
}
